...
class TestLogSourceMapper(TestCase):
    ...
    def test_logsource_validate_security_4688(self):
        ...
        self.assertFalse(ls.validate({"selection": {"Image": "a.exe"}}))
        self.assertFalse(ls.validate({"selection": {"ParentImage": "b.exe"}}))
        self.assertTrue(ls.validate({"selection": {"NewProcessName": "a.exe" }}))
        self.assertTrue(ls.validate({"selection": {"ParentProcessName": "b.exe" }}))
        self.assertTrue(ls.validate({"selection": {"NewProcessName|contains": "c.exe" }}))
        self.assertTrue(ls.validate({"selection": {'ParentProcessName|endswith': '\\winword.exe', 'NewProcessName|contains': '/l'}, 'condition': 'selection'}))

    def test_logsource_validate_sysmon_1(self):
        ls = LogSource(category="process_creation", event_id=1, service="", channel="")
        self.assertFalse(ls.validate({"selection": {"NewProcessName": "a.exe"}}))
        self.assertFalse(ls.validate({"selection": {"ParentProcessName": "b.exe"}}))
        self.assertTrue(ls.validate({"selection": {"Image": "a.exe" }}))
        self.assertTrue(ls.validate({"selection": {"ParentImage": "b.exe" }}))
        self.assertTrue(ls.validate({"selection": {"Image|contains": "c.exe" }}))
        self.assertTrue(ls.validate({'selection': {'Image|endswith': '\\winword.exe', 'CommandLine|contains': '/l'}, 'condition': 'selection'}))
